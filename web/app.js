...
const KEY='cse-field-guide-progress-v2';
const LEGACY_KEY='seven-cse-progress-v1';
...
const state={edition:'v2',view:'first14',year:'FY1',query:'',subject:'',status:'',semester:'',type:'',verification:'',limit:24};
...
function validateEntries(input){
  ...
  for(const [k,v] of Object.entries(input)){
    ...
    valid[k]={status:v.status,notes:v.notes,updated:typeof v.updated==='string'?v.updated:''};
  }return valid;
}
...
function load(){try{const raw=localStorage.getItem(KEY)||localStorage.getItem(LEGACY_KEY);if(raw){const p=JSON.parse(raw);if(![1,2].includes(p.schema))throw Error();progress=validateEntries(p.entries);learning=validateLearning(p.learning||{});}else{progress={};learning=validateLearning({});}}catch{showStorageError('Saved progress could not be read. Existing browser data has not been changed. Export your work before repairing storage.')}}
function setEdition(id){
  ...
  $('#edition').value=id;$('#edition-note').textContent=id==='v2'?'11,200 planned active hours':'Archived source · progress stays separate';
  if(id!=='v2'&&!['curriculum','resources','sources'].includes(state.view))state.view='curriculum';
  render();
}
...
function resourceResults(){
  ...
  $('#resource-count').textContent=`${resources.length} ${resources.length===1?'record':'records'}${state.year?' mapped to '+yearLabel(state.year):' in this edition'} · showing ${Math.min(resources.length,state.limit)}`;
...
function openUnit(id,tab='overview'){
  ...
  if(!routing)history.replaceState(null,'',`#/unit/${encodeURIComponent(id)}?edition=${state.edition}`);
...
function unitPanel(u){
  if(currentTab==='prerequisites')return relationPanel(u)+(state.edition==='v2'?moduleGatePanel(u.id):'')+sourceNote(u);
...
for(const kind of ['unit','resource'])$('#'+kind+'-dialog').addEventListener('close',()=>{
  if(!routing&&!$('dialog[open]')&&location.hash.startsWith('#/'+kind+'/')){
    const arg=state.view==='curriculum'&&state.year?'/'+state.year:'';
    history.replaceState(null,'',`#/${state.view}${arg}?edition=${state.edition}`);
  }
});
...
$('.brand').onclick=e=>{e.preventDefault();setEdition('v2');navigate('first14')};
...
async function init(){try{await loadCanonical();load();setEdition('v2');applyRoute()}catch(err){content.innerHTML=`<div class="empty"><h3>The curriculum could not be loaded.</h3><p>${esc(err.message)} Refresh to try again.</p></div>`}}
