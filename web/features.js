'use strict';
// Incremental V2.0 views share the existing cards, dialogs, navigation and styles.
...
async function loadCanonical(){
  const [curriculum,resources,practice,diagnostics,manifest,legacy]=await Promise.all(['curriculum','resources','practice','diagnostics','source-manifest','legacy-editions'].map(readData));
  ...
  const units=C.units.map(u=>({...u,raw:u,title:u.subject,subject:u.domain,originalYear:u.year,originalSemester:u.semester,year:yearId(u.year),semester:yearId(u.year)+'-'+u.semester.split('-').at(-1),level:`L${u.target}`,gate:active.get(u.id)?.assessment,resources:C.maps.filter(m=>m.unit_id===u.id).map(m=>({id:m.resource_id,role:m.role,scope:m.scope})),prerequisiteText:u.prerequisites.join(', ')||'None',prerequisiteKind:u.prerequisites.length?'explicit IDs':'none',source:`${MANIFEST.sourceVersion} · units · ${u.id}`,mapSource:`V2.0 maps and active_unit_view · ${u.id}`}));
  const canonical={id:'v2',name:'V2.0',label:'Canonical source',units,resources:resources.map(r=>({...r,url:r.direct_url||r.canonical_url||r.official_url,checked:r.last_checked,contentStatus:r.bibliography_status||r.full_text_status,priceStatus:r.cost_status,source:`${r.source_release||MANIFEST.sourceVersion} · ${r.source_sheet||'resources'} · ${r.source_row||r.id}`})),active:C.active,notes:'Complete 75-unit system. All source hours, units, prerequisites and resource records are preserved.'};
...
function navigate(view,arg=''){
  const next=`#/${view}${arg?'/'+encodeURIComponent(arg):''}?edition=${state.edition}`;
  if(location.hash===next)applyRoute();else location.hash=next;
}
function applyRoute(){
  if(!DATA)return;routing=true;
  try{
    const [path,search='']=location.hash.replace(/^#\/?/,'').split('?');const [requested,arg]=path.split('/').map(s=>decodeURIComponent(s));
    const edition=new URLSearchParams(search).get('edition')||'v2';
    ...
    if(state.edition!=='v2'&&!['curriculum','resources','sources'].includes(state.view))state.view='curriculum';
...
function resourceExtras(r){if(state.edition!=='v2')return '';
...
async function loadProvenance(id){const target=document.getElementById('provenance-'+id);if(!target)return;target.textContent='Loading provenance…';try{provenanceData=provenanceData||await readData('provenance');target.innerHTML=`<p>Raw historical records retain their original labels. Current V2.0 resource fields are shown above.</p><pre>${esc(JSON.stringify({aliases:provenanceData.aliases.filter(a=>a.resource_id===id),sourceRecords:provenanceData.provenance.filter(a=>a.resource_id===id)},null,2))}</pre>`;}catch{target.textContent='Provenance could not be loaded. Reopen the record to try again.'}}
function renderSources(){
  const audit=MANIFEST.sourceCounts;
  content.innerHTML=`<section class="source-panel about-brand"><img src="assets/logo.png" alt="CSE Field Guide" width="200" height="200"><div><h2>CSE Field Guide</h2><p><strong>Learn • Build • Explore • Become</strong></p><p>A rigorous seven-year self-directed Computer Science & Engineering education and mastery system.</p><p>Maintained by <a href="https://rishik.tech" target="_blank" rel="noopener noreferrer">Rishik Kumar Chaurasiya</a>.</p></div></section><div class="stats-strip"><span><strong>75</strong> units</span><span><strong>14</strong> semesters</span><span><strong>11,200</strong> active hours</span><span><strong>38</strong> labs</span><span><strong>24</strong> projects</span><span><strong>1</strong> specialization</span></div><section class="source-details"><h2>Canonical source</h2><p>${esc(MANIFEST.source)} · ${esc(MANIFEST.sourceVersion)}</p><p>Two full Foundation Years and five Engineering Years. The complete 75-unit system supersedes the incomplete 69-unit branch. Original hours and semester assignments are preserved.</p><p>${audit.resources} resource records form the catalogue. The active semester queue and exact unit scopes define what to study. Candidates, duplicates, discovery routes, and quarantined identities remain labelled.</p><p>Source gaps are retained; this app does not fill missing curriculum claims with generated content.</p></section><section class="source-details"><h2>Resource verification</h2>${detailFields(Object.entries(audit.verification))}<p>PAGE VERIFIED records visible identity, metadata, or official linkage. DIRECT ACCESS VERIFIED records retrieved content. Neither establishes playback, execution, current stock, or mastery. Other labels, dates, and inherited states are preserved verbatim in resource details. This website sync performs no new external-resource verification.</p></section><details class="source-details" open><summary>Known source limitations</summary><ul>${C.gaps.map(g=>`<li><strong>${esc(g[0])}:</strong> ${esc(g[1])}<br>${esc(g[2])}</li>`).join('')}</ul></details><section class="source-details"><h2>Your progress belongs to you</h2><p>Unit, First14, diagnostic, lab, and project records stay in this browser. Choose one specialization. Export backups before clearing browser storage or changing domains. Import validates a backup before merging it; matching imported records replace saved records.</p><p>Previous edition records are retained separately. No Gold unit is silently mapped to a V2.0 unit, and earlier completion is not automatically treated as V2.0 mastery.</p><div class="action-row"><button class="primary-button" data-export>Export progress</button><button data-import>Import backup</button></div></section><details class="source-details"><summary>Source history and integrity</summary><p>${MANIFEST.originalResourceIds.length} original resource IDs preserved. ${audit.maps} source mapping rows retained. Repeated mapping rows are grouped only for display. Source archive files are not republished.</p><p>Canonical data SHA-256: <code>${MANIFEST.sourceSha256}</code></p><p>Archived editions remain available through the Source edition selector, with separate local progress.</p><p><a href="data/source-views.json" download>Download supplementary source tables (JSON)</a>: original grouped resource snapshots, comparison notes, benchmark context, coverage audits, and planning allowances. These preserve source history; they are not an additional study checklist or current price quotations.</p></details><section class="source-details"><h2>Education and content boundaries</h2><p>This self-directed system awards no university degree, accredited credits, or professional registration. Completion requires independent learning evidence, experiments, and defense. It does not imply affiliation with any university.</p><p>Third-party books, courses, papers, videos, and documentation are linked and referenced; they are not republished as project-owned content. Their respective rights and access rules remain in force.</p><p>The intended project domain is <a href="https://cse.rishik.tech">cse.rishik.tech</a>. Domain connection is pending; this release does not change DNS or the portfolio.</p></section>`;
}
