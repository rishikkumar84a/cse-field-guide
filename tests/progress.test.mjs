...
async function app(saved={}) {
  ...
  const context=vm.createContext({
    ...
    fetch:async url=>({ok:true,json:async()=>JSON.parse(fs.readFileSync(url,'utf8'))})
  });
...
test('legacy progress survives rebrand without granting V2 mastery',async()=>{
  const saved={schema:1,entries:{'gold:UNIT-0001':{status:'completed',notes:'Prior evidence'},'checkpoint:U001':{status:'in-progress',notes:'Checkpoint evidence'}}};
  const a=await app({'seven-cse-progress-v1':JSON.stringify(saved)});
  assert.deepEqual(a.state().progress['gold:UNIT-0001'],{status:'completed',notes:'Prior evidence',updated:''});
  assert.equal(a.run('currentStatus("U001")'),'not-started');
  a.run('save()');
  assert.ok(a.storage.has('cse-field-guide-progress-v2'));
...
test('valid backup imports units, activities, First14, diagnostics and one track',async()=>{
  ...
  assert.match(a.run('esc(progress["v2:U001"].notes)'),/&lt;script&gt;/);
});
test('invalid imports are rejected atomically without replacing existing progress',async()=>{
  ...
  const before=a.state(),stored=a.storage.get('cse-field-guide-progress-v2');
  const invalid=[
    '{broken json',
  ...
  for(const backup of invalid){assert.match(await a.importBackup(backup),/Import stopped/);assert.deepEqual(a.state(),before);assert.equal(a.storage.get('cse-field-guide-progress-v2'),stored);}
});
test('unreadable saved data is protected from automatic overwrites',async()=>{
  const a=await app({'cse-field-guide-progress-v2':'{broken'});
  assert.match(a.state().storageError,/could not be read/);
  assert.equal(a.run('save()'),false);
  assert.equal(a.storage.get('cse-field-guide-progress-v2'),'{broken');
});
